...
既存設定ファイルの緩い権限を起動時に自動修復、非所有ファイルは読取前に拒否（Windows未対応）。
いずれも修正前のコードに戻すと新規テストが失敗することを確認済み。`security-review-v2.0.md`の対応状況欄も更新。）  
更新日: 2026-07-04 v10（Codex による `security-review-v2.0.md` を実施、Opusセッションで独立検証（3件とも正確、
severity妥当と確認）。**HENJI-SEC-001（MCPツール実行の個別承認/allowlist欠如、Medium、CWE-441）は
v2.0.0では対応保留と判断**。理由: (1) 修正規模が大きい（config.goスキーマ拡張+公開時フィルタ+実行時ゲート+監査ログ）、
(2) 別件でMCP関連コードのフットプリントが無視できない分量を占めることが判明し、
**MCP呼び出し機能自体を将来的に廃止する可能性も含めてユースケースを詳細検討する必要がある**ため、
allowlist機構に投資する前に方針を固めるのが合理的と判断。HENJI-SEC-002（上限判定がツール実行後、Medium、
リリースブロッカー相当）・HENJI-SEC-003（旧設定ファイル権限未修復、Low）の対応要否は別途判断。
詳細な検証結果とfinding別の修正計画はセッション記録を参照（本ファイルには要約のみ記載）。  
更新日: 2026-07-04 v9（`v2.0.0` 判断: モデル別利用能力テスト12/12達成を根拠に基準達成と判断。
module path を `forge.harakara.site/littleisland/henji/v2` に一括変更、
`git tag v2.0.0` をローカルに作成（origin へは未push）。詳細はセクション0「バージョニング方針」を参照）  
...
### R-F. プロンプトキャッシュ未対応（`-c`/`-C`継続時のコスト特性）

`-c <id>`/`-C`で会話を継続すると、`stream.go:51-52`（`setupStreamContext`）が
ローカルSQLiteキャッシュから会話全履歴を読み込み、新しい発言を1件追加した上で
**全履歴をまるごと毎回APIに再送**する（Anthropic/OpenAI/Google全APIがステートレス
なため、これが唯一の実装方法であり henji 固有の欠陥ではない）。

- 会話が伸びるほど入力トークン課金が線形〜準二次関数的に増える（N往復で合計コストは
  概ねO(N²)）
- `anthropic-sdk-go`は`CacheControlEphemeralParam`（プロンプトキャッシュ、直前と
  同じ接頭辞部分をキャッシュ読み取り価格＝概ね1/10のコストで再利用する機能）に
  既に対応しているが、`internal/anthropic/anthropic.go`は一切利用していない
  （2026-07-05確認、`grep`でヒットなし）
- OpenAIは同種のプロンプトキャッシュをサーバー側で自動適用するため、クライアント側の
  変更は不要（対応済みと同義）
- Googleは明示的なキャッシュ資源（CachedContent）を作成・参照する別APIが必要で、
  henjiの「伸びていく会話履歴」という利用パターンとは相性が悪い可能性がある
  （静的な大きいコンテンツを複数回参照する用途向き）

**対応の複雑さの見立て**（実装前の初期評価、未検証）:
- OpenAI: 対応不要（既に自動適用）
- Anthropic: `internal/anthropic/format.go`のメッセージ変換部分に`cache_control`
  マーカーを追加する、比較的閉じた変更。ただし考慮点がある——
  - キャッシュ有効期限（ephemeralは既定5分）を超えると効果がなく、書き込み自体は
    通常より高い（約1.25倍）ため、TTL内に再利用されないと逆に割高になる
  - モデルによって最小トークン数の閾値がある（下回ると無言でキャッシュされない
    可能性、要検証）
  - リクエストあたりのキャッシュブレークポイント数に上限がある
- Google: 別API（CachedContent作成・ライフサイクル管理）が必要でより重い変更。
  利用パターンとの相性も含めて費用対効果を要検討

出典: 2026-07-05、`--json-schema`実装後のFable 5レビュー依頼（henji経由で実際に
`-c`継続を使用）で気づいた運用上の特性。

**2026-07-04、着手を保留**: SDK対応（`TextBlockParam.CacheControl`）はコード変更として
軽いが、実際に効果を出すにはブレークポイント配置の設計判断（上限4個、システムプロンプト末尾
+最後から2番目のユーザーメッセージが定石）と、TTL5分・モデル別最小トークン閾値・書き込み
コスト1.25倍という特性上、henjiの対話的な`-c`利用パターンと相性が良いか未検証。効果測定の
